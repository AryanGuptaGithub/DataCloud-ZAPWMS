...
const STATUS_CLS = {
  ...
  cancelled: "bg-red-50 text-red-600 border-red-200 dark:bg-red-950/40 dark:text-red-300 dark:border-red-900",
};
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 p-4 md:p-6 space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Button variant="outline" size="sm" onClick={fetchIncomes} disabled={loading} className="h-8 text-xs border-gray-200 dark:border-gray-700">
          ...
          {TABS.map(({ id, label }) => (
            <button key={id} onClick={() => setTab(id)}
              className={`px-3 py-2 text-xs font-medium whitespace-nowrap border-b-2 transition-colors
                ${tab === id ? "border-emerald-600 text-emerald-700 dark:text-emerald-400" : "border-transparent text-gray-500 hover:text-gray-700 dark:hover:text-gray-300"}`}>
              {label}
            </button>
          ))}
        </div>
      ...
      {filtered.length === 0 ? (
        <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-xl p-16 text-center">
          ...
          <p className="text-sm font-medium text-gray-900 dark:text-white mb-1">No income records found</p>
            ...
            return (
              <div key={income.id}
                className={`group bg-white dark:bg-gray-900 border rounded-xl px-4 py-3 flex items-center gap-3 transition-all hover:shadow-sm
                  ${selected.has(income.id) ? "border-emerald-300 dark:border-emerald-700" : "border-gray-100 dark:border-gray-800"}`}>
                ...
                <div className="w-9 h-9 rounded-lg bg-emerald-50 dark:bg-emerald-950/40 flex items-center justify-center shrink-0">
                  <PayIcon className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
                </div>
                ...
                {/* Amount */}
                <p className="text-base font-bold text-emerald-600 dark:text-emerald-400 shrink-0 tabular-nums">
                  {INR(income.amount)}
                </p>
              ...
              return (
                <button key={p} onClick={() => setPage(p)}
                  className={`w-8 h-8 rounded-lg text-xs font-medium transition-colors
                    ${p === safePage ? "bg-emerald-600 text-white" : "text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800"}`}>
                  {p}
                </button>
              );
            })}
