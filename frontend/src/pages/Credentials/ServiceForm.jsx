    ...
    <div className="border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden">
      <button type="button" onClick={onToggle}
        ...
        <div className="flex items-center gap-2 min-w-0">
          ...
          {badge && (
            <span className="px-1.5 py-0.5 rounded-full text-xs bg-sky-100 text-sky-700 dark:bg-sky-950/40 dark:text-sky-300 shrink-0">
              {badge}
            </span>
          )}
          {hint && !open && (
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          {!open && (
            <span className="text-xs text-sky-600 dark:text-sky-400 font-medium">Click to expand</span>
          )}
...
function DynamicList({ label, values, onChange, placeholder = "Enter value" }) {
  ...
  return (
    <div className="space-y-1.5">
      ...
      {values.length === 0 ? (
        <button type="button" onClick={add}
          className="w-full py-2 border border-dashed border-gray-300 dark:border-gray-600 rounded-lg text-xs text-gray-400 hover:text-sky-600 hover:border-sky-400 transition-colors">
          + Add {label}
        </button>
      ) : (
        <div className="space-y-1.5">
          {values.map((v, i) => (
            <div key={i} className="flex items-center gap-2">
              <Input value={v} onChange={(e) => update(i, e.target.value)}
                ...
                className="h-8 text-xs border-gray-200 dark:border-gray-700 flex-1" />
  ...
  return (
    <div className="space-y-3">
      ...
      {domains.length === 0 ? (
        ...
        domains.map((d, i) => (
          <div key={i} className="border border-gray-200 dark:border-gray-700 rounded-lg p-3 space-y-3 bg-gray-50/50 dark:bg-gray-800/30">
            <div className="flex items-center gap-2">
              <Input value={d.domain} onChange={(e) => updateField(i, "domain", e.target.value)}
                placeholder="example.com" className="h-8 text-xs border-gray-200 dark:border-gray-700 flex-1" />
              <select value={d.type} onChange={(e) => updateField(i, "type", e.target.value)}
                className="h-8 text-xs border border-gray-200 dark:border-gray-700 rounded-lg px-2 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 shrink-0">
...
export default function ServiceForm({ initial, clientName, onSave, onCancel, saving }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 ">
            ...
            {["Domain", "Hosting"].map((t) => (
              <button key={t} type="button" onClick={() => set("type", t)}
                className={`flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-medium border transition-all
                  ${form.type === t
                    ? t === "Domain"
                      ? "bg-sky-50 border-sky-300 text-sky-700 dark:bg-sky-950/50 dark:border-sky-700 dark:text-sky-300"
                      : "bg-violet-50 border-violet-300 text-violet-700 dark:bg-violet-950/50 dark:border-violet-700 dark:text-violet-300"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div className="space-y-1.5">
            ...
            <Input value={form.service_name} onChange={(e) => set("service_name", e.target.value)}
              ...
              className={`h-8 text-sm ${errs.service_name ? "border-red-400" : "border-gray-200 dark:border-gray-700"}`} />
            <Err f="service_name" />
          </div>
          <div className="space-y-1.5">
            ...
            <Input value={form.provider} onChange={(e) => set("provider", e.target.value)}
              ...
              className={`h-8 text-sm ${errs.provider ? "border-red-400" : "border-gray-200 dark:border-gray-700"}`} />
            <Err f="provider" />
          </div>
          <div className="space-y-1.5">
            ...
            <Input value={form.portal_url} onChange={(e) => set("portal_url", e.target.value)}
              ...
              className={`h-8 text-sm ${errs.portal_url ? "border-red-400" : "border-gray-200 dark:border-gray-700"}`} />
            <Err f="portal_url" />
          </div>
          <div className="space-y-1.5">
          ...
          <div className="space-y-1.5 sm:col-span-2">
            ...
            <Input type="date" value={form.expiry} onChange={(e) => set("expiry", e.target.value)}
              className={`h-8 text-sm ${errs.expiry ? "border-red-400" : "border-gray-200 dark:border-gray-700"}`} />
            <Err f="expiry" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div className="space-y-1.5">
            ...
            <Input value={form.login} onChange={(e) => set("login", e.target.value)}
              placeholder="admin" className="h-8 text-sm border-gray-200 dark:border-gray-700" />
          </div>
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
            ...
            <div className="relative">
              <Input type={showPwd ? "text" : "password"} value={form.password}
                ...
                className="h-8 text-sm pr-8 border-gray-200 dark:border-gray-700" />
            ...
            {form.password && (
              <div className="space-y-1 pt-0.5">
                <div className="flex gap-0.5">
                  {[0,1,2,3,4].map((i) => (
                    <div key={i} className={`h-1 flex-1 rounded-full ${i < pwd.score ? pwd.bar : "bg-gray-200 dark:bg-gray-700"}`} />
                  ))}
                </div>
      ...
      >
        <p className="text-xs text-gray-400 bg-gray-50 dark:bg-gray-800 px-3 py-2 rounded-lg">
        ...
        {(form.meta?.renewal_history ?? []).length > 0 ? (
          <div className="space-y-1.5">
            {[...(form.meta.renewal_history)].reverse().map((r, i) => (
              <div key={i} className="flex items-center gap-3 text-xs px-3 py-2 bg-gray-50 dark:bg-gray-800 rounded-lg">
                <span className="font-medium text-gray-700 dark:text-gray-300">{r.date}</span>
                <span className="text-gray-400">·</span>
                <span className="text-gray-600 dark:text-gray-400">{r.duration}yr</span>
                {r.cost && <><span className="text-gray-400">·</span><span className="text-gray-600 dark:text-gray-400">₹{r.cost}</span></>}
      ...
      >
        ...
        {!advancedCount && (
          <div className="flex items-start gap-2.5 px-3 py-2.5 bg-sky-50 dark:bg-sky-950/30 border border-sky-200 dark:border-sky-900 rounded-lg">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              <p className="text-xs font-bold text-gray-600 dark:text-gray-300 uppercase tracking-wider flex items-center gap-2">
                <span className="w-4 h-px bg-gray-300 dark:bg-gray-600" />Server / VPS Details
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Input value={form.meta?.ip_address ?? ""} onChange={(e) => setMeta("ip_address", e.target.value)}
                    placeholder="192.168.1.1" className="h-8 text-sm border-gray-200 dark:border-gray-700" />
                </div>
                <div className="space-y-1.5">
                  ...
                  <Input value={form.meta?.server_details ?? ""} onChange={(e) => setMeta("server_details", e.target.value)}
                    placeholder="2 vCPU, 4GB RAM, Ubuntu 22..." className="h-8 text-sm border-gray-200 dark:border-gray-700" />
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <p className="text-xs font-bold text-gray-600 dark:text-gray-300 uppercase tracking-wider flex items-center gap-2">
                <span className="w-4 h-px bg-gray-300 dark:bg-gray-600" />Control Panel (CyberPanel / cPanel)
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1.5 sm:col-span-2">
                  ...
                  <Input value={form.meta?.panel?.url ?? ""} onChange={(e) => setPanel("url", e.target.value)}
                    placeholder="https://server.com:8090" className="h-8 text-sm border-gray-200 dark:border-gray-700" />
                </div>
                <div className="space-y-1.5">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input type={showPanel ? "text" : "password"} value={form.meta?.panel?.password ?? ""}
                    ...
                    placeholder="••••••••" className="h-8 text-sm border-gray-200 dark:border-gray-700" />
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <p className="text-xs font-bold text-gray-600 dark:text-gray-300 uppercase tracking-wider flex items-center gap-2">
                <span className="w-4 h-px bg-gray-300 dark:bg-gray-600" />DNS Records
              </p>
            ...
            <div className="space-y-3">
              <p className="text-xs font-bold text-gray-600 dark:text-gray-300 uppercase tracking-wider flex items-center gap-2">
                <span className="w-4 h-px bg-gray-300 dark:bg-gray-600" />Domain & Subdomain Mapping
              </p>
              <DomainMappingList
      ...
      <div className="flex justify-end gap-2 pt-1">
        <Button type="button" variant="outline" size="sm" onClick={onCancel} disabled={saving}
          className="h-8 text-xs border-gray-200 dark:border-gray-700">Cancel</Button>
