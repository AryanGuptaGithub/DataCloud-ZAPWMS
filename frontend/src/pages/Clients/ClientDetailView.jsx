...
/* ── helpers ─────────────────────────────── */
const CATEGORIES = {
  premium:  { label: "Premium",  cls: "bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950/40 dark:text-amber-300 dark:border-amber-800"    },
  regular:  { label: "Regular",  cls: "bg-sky-50 text-sky-700 border-sky-200 dark:bg-sky-950/40 dark:text-sky-300 dark:border-sky-800"                },
  lead:     { label: "Lead",     cls: "bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950/40 dark:text-emerald-300 dark:border-emerald-800" },
  inactive: { label: "Inactive", cls: "bg-gray-100 text-gray-600 border-gray-200 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-700"            },
  prospect: { label: "Prospect", cls: "bg-violet-50 text-violet-700 border-violet-200 dark:bg-violet-950/40 dark:text-violet-300 dark:border-violet-800" },
};
...
function InfoRow({ icon: Icon, label, value }) {
  ...
  return (
    <div className="flex items-start gap-2.5">
      <div className="w-7 h-7 rounded-md bg-gray-100 dark:bg-gray-800 flex items-center justify-center shrink-0 mt-0.5">
...
function CommLogForm({ clientId, onAdded }) {
  ...
  return (
    <form onSubmit={submit} className="space-y-3 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-100 dark:border-gray-800">
          ...
          return (
            <button key={t} type="button" onClick={() => setType(t)}
              className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-lg text-xs font-medium border transition-all
                ${type === t ? `${cls} bg-white dark:bg-gray-900 border-current` : "bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700 text-gray-500 hover:border-gray-300"}`}>
      ...
      <div className="flex items-start justify-between">
        <div className="min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <h2 className="text-base font-bold text-gray-900 dark:text-white">{client.clientName}</h2>
        ...
        <div className="flex items-center gap-1.5 shrink-0 ml-4">
          ...
          {onEdit && (
            <Button size="sm" onClick={onEdit}
              className="h-7 text-xs gap-1.5 bg-gray-900 hover:bg-gray-800 dark:bg-white dark:text-gray-900 text-white">
            ...
            <>
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {client.tags.map((t) => (
                    <span key={t} className="px-2 py-0.5 rounded-full text-xs bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-700">
                      {t}
                    </span>
                  ))}
                </div>
          ...
          {client.followUpDate && (
            <>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-start gap-2.5 p-3 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900 rounded-lg">
                  <Clock className="w-4 h-4 text-amber-600 shrink-0 mt-0.5" />
                  <div>
                    <p className="text-sm font-medium text-amber-800 dark:text-amber-300">{fmtDate(client.followUpDate)}</p>
                    {client.followUpNotes && (
                      <p className="text-xs text-amber-700 dark:text-amber-400 mt-0.5">{client.followUpNotes}</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                {(logsExp ? commLogs : commLogs.slice(0, 3))
                  ...
                  .map((log, i) => {
                    ...
                    return (
                      <div key={i} className="flex gap-3 p-3 bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-xl">
                        <div className={`w-7 h-7 rounded-md flex items-center justify-center shrink-0 bg-gray-50 dark:bg-gray-800 ${cls}`}>
                          <Icon className="w-3.5 h-3.5" />
                        </div>
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center justify-between gap-2">
                            <p className="text-xs font-semibold text-gray-700 dark:text-gray-300 capitalize">{log.type}</p>
                            <p className="text-xs text-gray-400 shrink-0">{fmtDate(log.date)}</p>
                          </div>
                          <p className="text-sm text-gray-700 dark:text-gray-300 mt-0.5">{log.summary}</p>
